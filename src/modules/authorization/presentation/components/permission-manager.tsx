...
export function PermissionManager() {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center bg-surface border border-border/10 rounded-2xl">
        ...
        <p className="text-muted-foreground text-sm max-w-sm">
          No tienes permisos para acceder a la gestión de roles y accesos. Solo los propietarios y administradores pueden ver esta sección.
        </p>
      </div>
    )
  }
  ...
  const handleToggle = async (
    ...
    const existing = permissions.find(p => p.roleId === roleId && p.collectionId === collectionId)
  ...
  return (
    <div className="space-y-12">
      {roles.filter(r => !r.is_superadmin).map(role => (
        <div key={role.id} className="bg-surface border border-border/5 rounded-2xl overflow-hidden">
            ...
            <Table>
              ...
              <TableBody>
                {collections.map(collection => {
                  const perm = permissions.find(p => p.roleId === role.id && p.collectionId === collection.id)
                  ...
                  return (
                    <TableRow key={collection.id}>
                      <TableCell className="font-medium">{collection.displayName || collection.name}</TableCell>
                      <TableCell className="text-center">
                        <Switch 
                          checked={canRead} 
                          onCheckedChange={() => handleToggle(role.id, collection.id, 'canRead', canRead)}
                        />
                      </TableCell>
                      <TableCell className="text-center">
                        <Switch 
                          checked={canCreate} 
                          onCheckedChange={() => handleToggle(role.id, collection.id, 'canCreate', canCreate)}
                        />
                      </TableCell>
                      <TableCell className="text-center">
                        <Switch 
                          checked={canUpdate} 
                          onCheckedChange={() => handleToggle(role.id, collection.id, 'canUpdate', canUpdate)}
                        />
                      </TableCell>
                      <TableCell className="text-center">
                        <Switch 
                          checked={canDelete} 
                          onCheckedChange={() => handleToggle(role.id, collection.id, 'canDelete', canDelete)}
                        />
                      </TableCell>
                    </TableRow>
                  )
