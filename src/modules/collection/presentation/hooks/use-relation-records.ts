...
export function useRelationRecords() {
  ...
  const searchRelations = useCallback(
    async (field: Field, query: string) => {
  ...
  const fetchBatchOptionsByIds = useCallback(
    async (tasks: { field: Field; ids: string[] }[]) => {
