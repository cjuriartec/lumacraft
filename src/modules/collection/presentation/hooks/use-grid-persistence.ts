...
import { PersistGridFiltersUseCase, LoadGridFiltersUseCase } from '../../application/use-cases/persist-grid-filters.use-case';
import { ColumnFilter } from '../../domain/types/pagination.types';
import { GridFiltersState } from '../../domain/ports/grid-state-repository.port';
...
export function useGridPersistence(collectionId: string | null) {
  ...
  const persistFilters = useCallback(async (filters: ColumnFilter[], rawValues: Record<string, string>, search: string) => {
