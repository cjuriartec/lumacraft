...
const InlineCombobox = ({
  ...
  const isCreator = React.useMemo(() => {
    ...
    const currentUserId = editor.meta.userId as string | undefined;
...
const InlineComboboxInput = ({
  ...
  return (
    <>
      {showTrigger && <span className="text-muted-foreground">{trigger}</span>}
...
const InlineComboboxContent: typeof ComboboxPopover = ({ className, ...props }) => {
  ...
  return (
    <Portal>
      <ComboboxPopover
        className={cn(
          "z-500 max-h-[288px] w-[300px] overflow-y-auto rounded-xl border border-border/50 bg-surface/80 backdrop-blur-xl p-1 shadow-2xl transition-all animate-in fade-in zoom-in-95",
          className,
        )}
...
const comboboxItemVariants = cva(
  "relative flex h-9 select-none items-center gap-2 rounded-lg px-2 text-sm font-medium outline-none transition-colors",
  {
    defaultVariants: {
      interactive: true,
    },
    variants: {
      interactive: {
        ...
        true: "cursor-pointer text-foreground hover:bg-surface-hover/50 data-[active-item=true]:bg-surface-hover data-[active-item=true]:text-primary",
      },
    },
  },
);
...
function InlineComboboxGroup({ className, ...props }: React.ComponentProps<typeof ComboboxGroup>) {
  return (
    <ComboboxGroup
      {...props}
      className={cn(
        "hidden not-last:border-b border-border/20 py-1 [&:has([role=option])]:block",
        className,
      )}
    />
  );
...
function InlineComboboxGroupLabel({
  ...
  return (
    <ComboboxGroupLabel
      {...props}
      className={cn(
        "mt-1.5 mb-1 px-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/70",
        className,
      )}
    />
  );
