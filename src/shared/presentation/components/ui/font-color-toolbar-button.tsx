...
import { Baseline, CheckIcon, EraserIcon, PlusIcon } from "lucide-react";
  ...
  const recordColorUsage = React.useCallback((color: string) => {
    ...
    setColorsQueue((prev) => {
      const filtered = prev
        ...
        .filter((c) => !DEFAULT_COLORS.some((dc) => normalizeColor(dc.value) === c));
        ...
        <ToolbarButton pressed={open} tooltip={tooltip}>
          {children || (
            <div className="flex flex-col items-center">
              ...
              <div
                className="mt-0.5 h-0.5 w-[90%] rounded-full opacity-90 transition-all border border-foreground/30 shadow-[0_0_0_1px_rgba(255,255,255,0.05)]"
                style={{ backgroundColor: selectedColor || color || "currentColor" }}
              />
            </div>
          )}
        </ToolbarButton>
...
function ColorCustom({
  ...
  const fullCustomColors = React.useMemo(
    () =>
      colorsQueue
        ...
        .filter((c) => !DEFAULT_COLORS.some((dc) => normalizeColor(dc.value) === normalizeColor(c)))
        .filter(
          (c) =>
            !DEFAULT_CUSTOM_COLORS.some((dc) => normalizeColor(dc.value) === normalizeColor(c)),
        )
        .map((c) => ({
  ...
  const isColorInCollections = React.useCallback(
    (targetColor: string) =>
      colors.some((c) => normalizeColor(c.value) === normalizeColor(targetColor)) ||
      customColors.some((c) => normalizeColor(c.value) === normalizeColor(targetColor)) ||
      fullCustomColors.some((c) => normalizeColor(c.value) === normalizeColor(targetColor)),
  ...
  const content = (
    <DropdownMenuItem
      className={cn(
        ...
        "my-1 flex size-6 items-center justify-center rounded-full p-0 transition-all hover:scale-125 border border-solid",
        isBrightColor
