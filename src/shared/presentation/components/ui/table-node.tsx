...
type TableResizeDirection = "left" | "right";
...
export const TABLE_DEFAULT_COLUMN_WIDTH = 56;
  ...
  const startResize = React.useCallback(
    (
      ...
      dragStateRef.current = {
        ...
        initialSize: effectiveColSizesRef.current[colIndex] ?? TABLE_DEFAULT_COLUMN_WIDTH,
      ...
      const handlePointerMove = (pointerEvent: PointerEvent) => applyResize(pointerEvent, false);
        ...
        <TableResizeContext.Provider value={resizeController}>
          <div ref={wrapperRef} className="group/table relative w-fit" style={tableVariableStyle}>
            <div
              ...
              className="-translate-x-[0.5px] pointer-events-none absolute inset-y-0 z-36 hidden w-px bg-primary/40"
              contentEditable={false}
            />
            <div
              ref={hoverIndicatorRef}
              className="-translate-x-[0.5px] pointer-events-none absolute inset-y-0 z-35 hidden w-px bg-primary/20"
              contentEditable={false}
            />
            <table
              ref={tableRef}
              className={cn(
                "mr-0 table h-px table-fixed border-separate border-spacing-0 bg-transparent border-0 rounded-sm",
...
function TableFloatingToolbar({ children, ...props }: React.ComponentProps<typeof PopoverContent>) {
  ...
  const isExpandedSelectionPending = isFocusedLast && !collapsedInside && selectedCellCount > 1;
  ...
  const isToolbarOpen = isCollapsedToolbarOpen || shouldRenderExpandedSelectionToolbar;

  return (
    <Popover open={isToolbarOpen} modal={false}>
      <PopoverAnchor asChild>{children}</PopoverAnchor>
      {isCollapsedToolbarOpen && <CollapsedTableFloatingToolbarContent {...props} />}
...
function TableFloatingToolbarContent({
  ...
  return (
    ...
    >
      <Toolbar
        className="scrollbar-hide flex w-auto max-w-[80vw] flex-row overflow-x-auto rounded-lg border-border/50 bg-surface/95 p-1 shadow-[0_2px_10px_rgba(0,0,0,0.05)] backdrop-blur-sm print:hidden"
        contentEditable={false}
      >
        <ToolbarGroup>
          <ColorDropdownMenu tooltip="Background color">
            <PaintBucketIcon className="text-muted-foreground transition-colors group-hover:text-foreground" />
          </ColorDropdownMenu>
          {canMerge && onMerge && (
            <ToolbarButton
              onClick={onMerge}
            ...
            >
              <CombineIcon className="text-muted-foreground transition-colors group-hover:text-foreground" />
            </ToolbarButton>
          )}
          {canSplit && onSplit && (
            <ToolbarButton
              ...
              className="hover:-translate-y-0.5 transition-all duration-200"
            >
              <SquareSplitHorizontalIcon className="text-muted-foreground transition-colors group-hover:text-foreground" />
            </ToolbarButton>
          )}
          <DropdownMenu modal={false}>
            <DropdownMenuTrigger asChild>
              <ToolbarButton
                ...
                className="hover:-translate-y-0.5 transition-all duration-200"
              >
                <Grid2X2Icon className="text-muted-foreground transition-colors group-hover:text-foreground" />
              </ToolbarButton>
            </DropdownMenuTrigger>
            <DropdownMenuPortal>
          ...
          {collapsedInside && (
            <ToolbarGroup>
              <ToolbarButton
                ...
                className="hover:-translate-y-0.5 transition-all duration-200"
              >
                <Trash2Icon className="text-muted-foreground transition-colors group-hover:text-foreground" />
              </ToolbarButton>
            </ToolbarGroup>
          )}
        </ToolbarGroup>
        {collapsedInside && (
          <ToolbarGroup>
            <ToolbarButton
              ...
              className="hover:-translate-y-0.5 transition-all duration-200"
            >
              <ArrowUp className="text-muted-foreground transition-colors group-hover:text-foreground" />
            </ToolbarButton>
            <ToolbarButton
              ...
              className="hover:-translate-y-0.5 transition-all duration-200"
            >
              <ArrowDown className="text-muted-foreground transition-colors group-hover:text-foreground" />
            </ToolbarButton>
            <ToolbarButton
              onClick={onDeleteRow}
              onMouseDown={(e) => e.preventDefault()}
              ...
              className="hover:-translate-y-0.5 transition-all duration-200"
            >
              <XIcon className="text-muted-foreground transition-colors group-hover:text-foreground" />
            </ToolbarButton>
          </ToolbarGroup>
        )}
        {collapsedInside && (
          <ToolbarGroup>
            <ToolbarButton
              ...
              className="hover:-translate-y-0.5 transition-all duration-200"
            >
              <ArrowLeft className="text-muted-foreground transition-colors group-hover:text-foreground" />
            </ToolbarButton>
            <ToolbarButton
...
function RowDragHandle({ dragRef }: { dragRef: React.Ref<HTMLButtonElement> }) {
  ...
  return (
    <Button
      ...
      variant="ghost"
      className={cn(
        "-translate-y-1/2 absolute top-1/2 left-0 z-51 h-6 w-4 p-0 focus-visible:ring-0 focus-visible:ring-offset-0 transition-opacity duration-200",
        "cursor-grab active:cursor-grabbing",
        "opacity-0 group-hover/row:opacity-100 group-data-[table-resizing=true]/row:opacity-0",
      )}
      onClick={() => editor.tf.select(element)}
    >
      <GripVertical className="text-muted/60 transition-colors hover:text-primary" />
    </Button>
  );
}
...
export function TableCellElement({
  ...
  return (
    <PlateElement
      ...
      as={isHeader ? "th" : "td"}
      className={cn(
        "relative h-full overflow-visible border-none p-0 font-normal",
        !element.background && isHeader && "bg-muted/10",
        isHeader &&
          "text-left font-semibold text-[11px] uppercase tracking-[0.12em] text-muted-foreground *:m-0",
        "before:size-full",
        "data-[table-cell-selected=true]:before:z-10",
        "data-[table-cell-selected=true]:before:bg-primary/5",
        "before:absolute before:box-border before:select-none before:content-['']",
        borders.bottom?.size && "before:border-b before:border-b-primary/25",
        borders.right?.size && "before:border-r before:border-r-primary/25",
        borders.left?.size && "before:border-l before:border-l-primary/25",
        borders.top?.size && "before:border-t before:border-t-primary/25",
      )}
      style={
        {
          maxWidth: width,
      ...
      attributes={{ ...props.attributes, colSpan, "data-table-cell-id": element.id, rowSpan }}
    >
      {element.background && (
        <div
          className="pointer-events-none absolute inset-0 z-0 bg-(--cellBackground) dark:opacity-[0.25] dark:saturate-[1.5]"
  ...
  return (
    ...
    >
      <div
        ...
        onPointerLeave={() => clearResizePreview(rightHandleKey)}
        onPointerDown={(event) =>
          startResize(event, { colIndex, direction: "right", handleKey: rightHandleKey, rowIndex })
        }
      />
      {isLeftHandle && (
        <div
          ...
          onPointerLeave={() => clearResizePreview(leftHandleKey)}
          onPointerDown={(event) =>
            startResize(event, { colIndex, direction: "left", handleKey: leftHandleKey, rowIndex })
          }
        />
      )}
    </div>
