...
Dado que es inaceptable tener tiempos de carga lentos (*Cold Starts* en Render/Railway) y necesitamos una solución integral que también cubuya almacenamiento (Storage) para los documentos generados, la estrategia definitiva es **reemplazar el Backend tradicional (NestJS) por Supabase**.
...
*   *Adiós a los Cold Starts*: Supabase no suspende tu servidor cada 15 minutos (a diferencia de Render). Solo pausa el proyecto si pasa **1 semana entera sin actividad**, lo cual no ocurrirá si lo estás usando activamente o lo pingeas.
...
*   *Google Gemini Free Tier*: Sigue siendo el cerebro oficial (1 millón de tokens por minuto gratis).
